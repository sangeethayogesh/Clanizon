  ...
  var leadItem=[];
  var  advance = (props.leadDetail && props.leadDetail.leadAudit)?(props.leadDetail.leadAudit[0]?props.leadDetail.leadAudit[0].advance : 0):0

  var  remaining = (props.leadDetail && props.leadDetail.leadAudit)?(props.leadDetail.leadAudit[0]?props.leadDetail.leadAudit[0].remaining : 0):0
  var leadItemnew=[];
  ...
  const onDataChange = (value) => {
    ...
    value.map((item) => {
      console.log(item);
      ordervalue=ordervalue+(item.qty*item.unitPrice)
      
    })
    remaining=ordervalue-advance;
    form.setFieldsValue({"remaining":remaining})
    
  }

  const onChangeAdvance=(value)=>{
    form.setFieldsValue({"remaining":remaining-value})
  }
